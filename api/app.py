...
async def predict(request: Request):
    try:
        ...
        ordered_feats = np.array([feats_dict.get(f, 0.0) for f in names], dtype=float)
        ordered_feats = np.nan_to_num(ordered_feats, nan=0.0, posinf=0.0, neginf=0.0)

        # Sanity check
        if ordered_feats.shape[0] != len(names):
            return jsonify({"error": f"Feature mismatch: got {ordered_feats.shape[0]}, expected {len(names)}"}), 500
